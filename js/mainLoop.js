...
function updateTimerAndTimelineLogic(deltaTimeMs) {
    ...
    // console.log(`[Loop] Checking movement. isMovingRight: ${state.isMovingRight}`); // Opzionale, può essere molto verboso

    if (state.isMovingRight) {
        // *** LOG INTERNO AL BLOCCO ***
        console.log(`[Loop] Updating time/width. Delta: ${deltaTimeMs.toFixed(1)}ms`);
        ...
        if (state.currentWidthPx < maxWidthPx) {
            ...
            state.currentWidthPx = Math.min(state.currentWidthPx + widthIncrement, maxWidthPx);
             ...
             // *** LOG VALORI AGGIORNATI ***
             console.log(`[Loop] New time: ${state.totalHoldTimeSec.toFixed(3)}s, New width: ${state.currentWidthPx.toFixed(1)}px`);
        } else {
            // *** LOG LARGHEZZA MASSIMA RAGGIUNTA ***
             console.log(`[Loop] Max width reached (${maxWidthPx}px). Time: ${state.totalHoldTimeSec.toFixed(3)}s`);
        }
    }
}
...
function handleTimelineEvents() {
    ...
    for (const event of config.timelineEvents) {
        if (event.type !== 'title' &&
            state.totalHoldTimeSec >= event.start && state.totalHoldTimeSec < event.end) {
             activeEvent = event;
             break;
        }
    }
    ...
    if (activePromptId !== undefined) {
         updatePromptDisplay(activePromptText, activePromptId);
    }
     if (activeAnimationId !== undefined) {
         updateAnimationDisplay(activeAnimationId);
     }
}
...
function animationLoop(timestampMs) {
    ...
    // Aggiorna prima la logica di stato
    updateTimerAndTimelineLogic(deltaTimeMs);

    // Poi aggiorna la UI basata sul nuovo stato
    updateTimerDisplay();
